...
//Check for user's followers
$follower_query = "select * from follow 
					where followedname = '" . $loginuser . "'";
$follower_result = $connection->query($follower_query) or die('Error! ' . $connection->error);

if (!$follower_result) {
	echo "Query not sucessful.";
} else {
	echo '<div class="border">';
	echo '<h2>Those Who I Follow...</h2>';
	echo '</div>';
	echo '<div class="w3-row-padding w3-margin-top">';
	while ($row = $follower_result->fetch_assoc()) {
		echo '<div class="w3-quarter">';
		echo '<div class="w3-card">';
		echo '<a href="user.php?uname=' . $row['followername'] . '"><img src="img/img8.jpg" alt="Person" class="w3-sepia-min" style="width:100%; height:80%"></a>';
		echo '<div class="w3-container" style="text-align:center>';
		echo '<a style="color:snow" href="user.php?uname=' . $row['followername'] . '">' . $row['followername'] . '</a>';
		echo '</div>';
		echo '</div>';
		echo '</div>';
	}
...
$follower_result->close();

//Check for who follows user
$followed_query = "select * from follow 
					where followername = '" . $loginuser . "'";
$followed_result = $connection->query($followed_query) or die('Error! ' . $connection->error);

if (!$followed_result) {
	echo "Query not sucessful.";
} else {
	echo '<div class="border">';
	echo '<h2>Those Who Follow Me...</h2>';
	echo '</div>';
	echo '<div class="w3-row-padding w3-margin-top">';
	while ($row = $followed_result->fetch_assoc()) {
		echo '<div class="w3-quarter">';
		echo '<div class="w3-card">';
		echo '<a href="user.php?uname=' . $row['followedname'] . '"><img src="img/img8.jpg" alt="Person" class="w3-sepia-min" style="width:100%; height:80%"></a>';
		echo '<div class="w3-container" style="text-align:center>';
		echo '<a style="color:snow" href="user.php?uname=' . $row['followedname'] . '">' . $row['followedname'] . '</a>';
		echo '</div>';
		echo '</div>';
		echo '</div>';
	}
