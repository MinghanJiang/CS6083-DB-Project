...
// Retrieve info. from track
$track_query = "select *
				from track a, tracklist b, artist c, album d, tracklist e
				where a.tid = b.tid
				and a.aid = c.aid
				and a.tid = e.tid
				and d.albumid = e.albumid
				and a.tid = '" . $trackid . "'";
$track_result = $connection->query($track_query) or die('Error! ' . $connection->error);

// Retrieve playlist that contains the track
$contain_playlist_query = "select c.* 
						from track a, pttracklist b, playlist c 
						where a.tid = b.tid
						and b.pid = c.pid
						and a.tid = '" . $trackid . "'";
$contain_playlist_result = $connection->query($contain_playlist_query) or die('Error! ' . $connection->error);

//Display all tracks and album info.
if (!$track_result || !$contain_playlist_result) {
	echo "Query not sucessful.";
} else {
	$row = $track_result->fetch_assoc();
	$albumid = $row['albumid'];
	echo '<div class="w3-twothird">';
	echo '<h3>' . $row['ttitle'] . '</h3>';
	$_SESSION['tid'] = $trackid;
	echo '<table style="color:white">';
	echo '<tr>';
	echo '<td>Artist:</td>';
	echo '<td><a href="artist.php?aid=' . $row['aid'] . '">'	. $row['aname'] . '</a></td>';
	echo '</tr>';
	echo '<tr>';
	echo '<td>Album:</td>';
	echo '<td><a href="album.php?albumid=' . $row['albumid'] . '">'	. $row['albumtitle'] . '</a></td>';
	echo '</tr>';
	echo '</table>';
	echo '<a target="_blank" href="playhistory.php?tid=' . $row['tid'] . '&albumid=' . $row['albumid'] . '">Play</a>';
	echo '</div>';
	
	$i=0;
	echo '<div class="w3-third w3-right">';
	echo '<div class="border">';
	echo '<h2 class=>Playlist that contains this track</h2>';	
	echo '</div>';
	echo '<div class="w3-row-padding w3-margin-top">';
	while ($i < 2 && $row = $contain_playlist_result->fetch_assoc()) {
