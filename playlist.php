...
// Retrieve info. and artist of the playlist
$playlist_query = "select *
				from playlist
				where pid = '" . $pid . "'";
$playlist_result = $connection->query($playlist_query) or die('Error! ' . $connection->error);

// Retrieve tracklist of the playlist
$tracklist_query = "select c.*, d.aname, e.albumid, e.albumtitle
					from playlist a, pttracklist b, track c, artist d, album e
					where a.pid = b.pid
					and b.tid = c.tid
					and c.aid = d.aid
					and b.albumid = e.albumid
					and a.pid = '" . $pid . "'";
$tracklist_result = $connection->query($tracklist_query) or die('Error! ' . $connection->error);

//Display all tracks and playlist info.
if (!$playlist_result || !$tracklist_result) {
	echo "Query not sucessful.";
} else {
	$row = $playlist_result->fetch_assoc();	
	echo '<h3>' . $row['ptitle'] . '</h3>';
	echo '<table style="color:white">';
	echo '<tr>';
	echo '<td>Creator:</td>';
	echo '<td><a href="user.php?uname=' . $row['uname'] . '">'	. $row['uname'] . '</td>';
	echo '</tr>';
	echo '<tr>';
	...
	echo '</tr>';
	echo '</table>';
	echo '<p>Description:' . $row['pdesc'] . '</p>';
