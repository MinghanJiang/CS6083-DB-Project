...
// Retrieve info. of the all matched search result
$search_query = "call get_match_result('" . $search_content . "')";
$search_result = $connection->query($search_query) or die('Error! ' . $connection->error);

//Display at most 20 records of the search results
if (!$search_result) {
	echo "Query not sucessful.";
} else {
	echo '<h3>Search Result for "' . $search_content . '"</h3>';
	...
	echo '</tr>';
	$i=0;
	while ($i < 20 && $row = $search_result->fetch_assoc()) {
		echo '<tr>';
